...
(1) $\frac{dR}{dt}=D(Rin-R)-\mu RN$

(2) $\frac{dN}{dt}=\mu RN - DN$
...
(3) $\frac{dR}{dt}=D(Rin-R)-\mu RN$

(4) $\frac{dN}{dt}=\mu RN -\alpha N+\beta S-  DN$

(5) $\frac{dS}{dt}=\alpha N-\beta S - DS$
...
(6) $\frac{dR}{dt}=D(Rin-R)-\mu RN$

(7) $\frac{dN}{dt}=\mu RN -\alpha N+\beta S-  DN$

(8) $\frac{dI1}{dt}=\alpha N -I1 - DI1$

(9) $\frac{I2}{dt}=I1 -I2 - DI2$

(10) $\frac{dS}{dt}=I2-\beta S - DS$
...
##Loss of sporulaton
In the next model a mutation enables the loss of sporulation. Beside sporulation there is no change in parameters of the mutant compared with the WT sporulator. The model is the single step sporulation model (no intermediates). the mutation rate to loose sporulation is 10^-6. 
...
chemostat1.mut <- function(t,current,params){
  with(as.list(c(params,current)), {
    ...
    dN <- mu * R * N - D * N - alpha * N + beta * S - loss * N
...
# all parameters together
params1 <- c(R_in, D, mu, alpha, beta, loss)
#declare initial conditions for state variables
...
(warning message is beacuse the system was intited with Nmut=0, which doesn't work well on log scale)
